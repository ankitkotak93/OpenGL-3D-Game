...
void printtext(int x, int y, char String[300]);
using namespace std;
void draw_cube();
void Set_Camera();
...
void drawScene() {
	...
	char buf[300];
	sprintf( buf,"Your  score: %f", score ) ;
	//const char * p = buf ;
...
float calc_sq(float a,float b, float c,float d) 
{ 
...
void update() 
{
	...
	prevpitch=pitch;
	if(vel>0)
		vel=acc+ret-(0.001*sinf(DEG2RAD(pitch)));
	else //if(vel==0)
	{
		ret=0;
		// 
		//if(pitch<0)
		// {
		vel=acc-(0.001*sinf(DEG2RAD(pitch)));
		//  }
		//  else
		//vel=vel+acc;
	}
	...
	if(ifrolled)
		if(vel!=0 && vel>0.05)
		{
			yaw+=-0.5*tanf(DEG2RAD(roll))/vel;

		}

	for(unsigned int i=0;i<fossil.size();i++) 
	{ 
		if(sqrt(calc_sq(bike_x,fossil[i].first,bike_z,fossil[i].second)) < 1) 
		{ 
			score++; 
...
void draw_cube()
{
	...
	glVertex3f(-0.125,0.125,0.0);
	glVertex3f(-0.375,0.375,0.0);
	glVertex3f(0.375,0.375,0);
	glVertex3f(0.55,0.375,0);
	glVertex3f(-0.375,0.375,0);
	glVertex3f(-0.55,0.375,0);
	glEnd();
	glPopMatrix();
	...
	glVertex3f(0.375,0.375,0.0);
	glVertex3f(0.125,0.125,0.0);	
	glVertex3f(0.375/1.414,0.125,0.0);	
	glVertex3f(-0.125,0.125,0.0);
	glVertex3f(-0.375,0.375,0.0);
	glVertex3f(-0.125,0.125,0.0);	
	glVertex3f(-0.375/1.414,0.125,0.0);	
	...
	glLineWidth(9);
	glBegin(GL_LINES);
	glVertex3f(0.225,0.225,0.0);
	glVertex3f(0.350,0.350,0.0);
	glVertex3f(-0.225,0.225,0.0);
	glVertex3f(-0.350,0.350,0.0);
	glEnd();
	glPopMatrix();
